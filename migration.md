...
### Этап 2: Изменение порта X-ray (VLESS)

Сначала мы освободим порт `443`, переместив сервис X-ray.

1.  **Войдите в панель 3x-ui** через браузер по старому адресу: `https://pgserver.online:8443/xray/`.
2.  Перейдите в раздел **"Inbounds"**.
3.  Найдите ваше входящее подключение, которое использует порт `443` (скорее всего, с пометкой "VLESS-Reality").
4.  Нажмите **"Edit"** (редактировать).
5.  В поле **"Port"** измените значение с `443` на `43443`.
6.  Нажмите **"Update"** (обновить) и убедитесь, что панель применила изменения. X-ray автоматически перезапустится с новым портом.
